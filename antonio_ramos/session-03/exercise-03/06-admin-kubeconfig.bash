...
#kubectl config set-cluster k8s-training-secondary --certificate-authority=$ADMIN_CLUSTER_CA_PATH --embed-certs=true --server=https://${CONTROLLER1_PUBLIC_IP}:6443
#kubectl config set-credentials k8s-training-admin --client-certificate=$ADMIN_CERT_PATH --client-key=$ADMIN_KEY_PATH --embed-certs=true
#kubectl config set-context k8s-training-admin --cluster=k8s-training-secondary --user=k8s-training-admin
